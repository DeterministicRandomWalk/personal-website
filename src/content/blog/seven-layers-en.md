---
title: 'Part 2: Why One Classifier Became Seven Layers'
...
The previous essay described an impossible disclosure task: a large investment workflow needed complete information at portfolio scale. A chatbot could not do it because scale, completeness, and auditability all failed. The problem therefore became an engineering problem.
...
The requirement was straightforward to state: can we systematically collect all relevant pages and files from the official websites of a large company universe?
...
“Decide which links to collect” takes three seconds to say and substantial engineering to make reliable.
...
A few websites later, it already fails.
...
The most obvious first step is to send each URL to the model individually: “Is this relevant?” Simple. Accurate. And immediately, absurdly expensive. A large corporate site can expose thousands of candidate links. Now repeat that across an entire portfolio.

So you batch many URLs into one prompt: “Which of these are ESG-related?” Costs drop by orders of magnitude. But how large should a batch be? Too small, and API calls are still too numerous. Too large, and the model starts losing track: classification quality drops, items are missed, output formatting breaks.

The best batch size is not something you can guess. You test several sizes, check accuracy, cost, and timeout rates, then keep adjusting.
...
For a large website, this means many batches and costs that accumulate quickly. Sometimes an API call times out halfway through processing. Sometimes the response format is malformed: the model omits a few items, and you do not know which ones are missing. Sometimes it simply misclassifies.
...
Not because AI is useless. AI is useful, and becoming more useful. But there is a long distance between “impressive in a demo” and “stable at portfolio scale across many languages on real data.” You do not cross that distance with hope. You test, measure, and hit the boundary again and again, even as the boundary keeps moving.
...
The extraction code itself? An AI coding assistant can write it efficiently. It is direct DOM traversal. But the insight that raw URLs are not enough, that surrounding text carries the signal, comes from repeatedly watching the classifier fail on real websites.
...
Now you are sending paragraph-level context for thousands of URLs per company. At that point, classification alone can consume millions of tokens before the system has collected a single document. Accurate classification cannot rely on the URL alone, but adding the necessary context creates a serious cost problem at portfolio scale.
...
Now a `/sustainability/` branch with many children is clearly worth exploring, while a large `/careers/` branch is not.
...
For large websites, this can remove a substantial share of URLs before the model sees anything.
...
Imagine a large dictionary where every word means something different depending on the company website. That is what this layer maintains.
...
This is the part nobody expects to require so many iterations.
...
You are many versions in, even though you expected the second attempt to be “done.”
...
An agent starts from the generic version every time unless someone encodes those improvements into stable prompt templates. At that point, the accumulated learning has become engineering.
...
Some companies generate thousands of ESG-related URLs. You cannot collect them all. But hard caps are dangerous: if a long run of low-value news appears before the real report, the cap misses the important content.
...
Commercial systems often reveal the same underlying reality. A collection engine handles repeatable work, while ambiguous or incomplete cases still need human review. The important point is not that automation failed. It is that completeness requires an explicit fallback rather than a claim of effortless autonomy.

## The Result

After deterministic filtering, the model sees only a small ambiguous remainder. Classification becomes cheap enough to repeat, inspect, and improve.
...
Chinese version: [read Part 2 in Chinese](/blog/seven-layers-zh/).
